...
**Stack**: static HTML + CSS, generated by a small Python build. The only
JavaScript on the site is a copy-to-clipboard button on blog code blocks
(`assets/js/blog-code-copy.js`); the main page ships none.
...
│   ├── 404.html.j2                  #   not-found page (root-absolute URLs)
│   ├── blog/
...
A header toggle switches between two themes:

- **Dark** — the default on every page load.
- **Light** — while the toggle is off.

It is implemented in pure CSS: a single checkbox plus `body:has(#theme-toggle)`
rules in `styles/main.css`. No JavaScript is involved.

Two known limitations follow from that CSS-only approach:

- The system `prefers-color-scheme` preference is not honored. The
  `@media (prefers-color-scheme: dark)` block near the top of `main.css` is
  currently dead code, since `body:has(#theme-toggle)` always matches and
  overrides it.
- The choice does not persist. Navigating from the home page to a post resets
  the toggle to dark.

Fixing either would require JavaScript or a rethink of the toggle.
...
- **Content**
  - Rewrite About, Projects, and Experience copy in `templates/index.html.j2`.
    The Experience timeline and the three project cards are still placeholder
    content.
  - Update project cards: titles, roles, years, and links (the `href="#"`
    links are not yet wired to anything).
  - Edit blog metadata and content in `content/blog/*.md`.
- **Contact**
  - Email, GitHub, and LinkedIn are set in the contact section.
- **Assets**
